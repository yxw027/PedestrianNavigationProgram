...
% 				[Kalx_f, KalP_f, gam] = filthif_upd(zz, H, R, Kalx_p, KalP_p, 5);
        ...
        est_pos2(est_counter,:)=est_pos1(est_counter,:)+INS1(est_counter,:); 
        est_pos3(timetag-1,:)=enu2xyz(est_pos2(timetag-1,1:3)',refl);
        est_posllh(timetag-1,:)=xyz2llh(est_pos3(timetag-1,1:3))*180/pi;
...
%                                                       
Data = [Result.ppp.time(:,5:10) Result.ppp.pos(:,4:6)];                      %Y M D H M S lat lon alt
output_kml(fn_kml,Data,track_color,point_color);
