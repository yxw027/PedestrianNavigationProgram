...
Result.float.dpwv(1:tt,1:2)=NaN;												% 可降水量
...
while 1
	...
	if sf == 0
		time1.mjd = -1e10;
		time2.mjd = -1e10;
		while time_s.mjd + 0.0001> time1.mjd+0.1/86400		%10秒ずらす(30秒なら0.0001)											% 約 0.1 秒ｽﾞﾚまで認める
			%--- エポック情報取得(時刻, PRN, Dataなど)
			%--------------------------------------------
			[time1,no_sat1,prn.rov.v,dtrec1,ephi1,data1]=...
					read_obs_epo_data(fpo1,eph_prm.brd.data,no_obs1,TYPES1);

			if time_s.mjd + 0.0001<= time1.mjd+0.1/86400, sf=1; break;, end
		end
		while time_s.mjd > time2.mjd+0.1/86400		%10秒ずらす	(+ 0.02)									% 約 0.1 秒ｽﾞﾚまで認める
			%--- エポック情報取得(時刻, PRN, Dataなど)
			%--------------------------------------------
...
% 		end
		...
		%--- 読み取り中のエポックの時間表示
		%--------------------------------------------
		fprintf('%7d: 河南-%2d:%2d %5.2f , 河内長野-%2d:%2d %5.2f"  ',timetag,time1.day(4),time1.day(5),time1.day(6),time2.day(4),time2.day(5),time2.day(6));
		...
		if ~isnan(x1(1)) & length(prn.c)>3
			for nn=1:est_prm.iteration
				...
				for k = 1:length(prn.c)
                    ...
                    %time2.day(6) = abs(time2.day(6) - 30);
...
% 				[Kalx_f, KalP_f] = filtsrcf_upd(zz, H, R, Kalx_p, KalP_p);
		...
		if ~isempty(zz)
			...
			case 5
                ionsize = size(Kalx_f(ix.i));
				Result.float.dion(timetag,1:ionsize(1,1))=Kalx_f(ix.i);										% 電離層遅延
			end
			if est_prm.statemodel.trop~=0
				Result.float.dtrop(timetag,:)=Kalx_f(ix.T);											% 対流圏遅延
            end
            %対流圏遅延（可降水量）
            if est_prm.statemodel.trop == 1
				Result.float.dpwv(timetag,:)=(1*10^5/(461*(71.98-77.6*18.0152/28.9644+(3.754*10^5)/22))*Result.float.dtrop(timetag,1))*1000;											% 対流圏遅延
            end
...
% 		if change_flag==1, fprintf(' , Change');, end
		...
		time_o=time1;
		timetag_o=timetag;
        %est_prm.refpos = Result.float.pos(timetag,1:3)';
	end
	%--- end 判定
	%--------------------------------------------
