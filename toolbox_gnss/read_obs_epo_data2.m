function [time, no_sat, prn, dtrec, ephi, data] = read_obs_epo_data(fpo, est_prm, Eph_mat, no_obs, TYPES)
%-------------------------------------------------------------------------------
...
while 1
	...
	if no_sat ~= 0
		...
		%--------------------------------------------
		neg_sat = [];
		ephi = repmat(NaN,1,61);											% 1-31(GPS), 38-61(GLONASS)
		neg_sat_num = 0;
		...
		for k=1:no_sat
			...
			ephi(prn(k)) = eph_search(est_prm, Eph_mat, prn(k), sys(k), time);
		...
		%--------------------------------------------
		if est_prm.n_nav==0
		data(find(sys==1),:)=[]; prn(find(sys==1))=[];
		end
		if est_prm.g_nav==0
		data(find(sys==2),:)=[]; prn(find(sys==2))=[];
		end
		data(find(37>prn>32),:)=[]; prn(find(37>prn>32))=[];
		no_sat=length(prn);

		if est_prm.n_nav==1 & est_prm.g_nav==0
		data(:,2) = data(:,2) + p1c1bias(time.day,prn);						% P1C1 DCB 補正(GPSのみ対応)
		end
...
function column = eph_search(est_prm,Eph_mat,prn, sys,tt)
%-------------------------------------------------------------------------------
% eph_search: read_eph で作成された Eph_mat 行列から適切な列を選択する
%
% [argin]
% Eph_mat : エフェメリスデータ
% prn     : 指定衛星番号
...
% 		if Eph_mat(6,column)==59										% エラー処理テスト
% 			column=0													% 最適なエフェメリスが無いとする
% 		end
	end
end

if est_prm.g_nav==1 & sys==2
	index = find(Eph_mat(1,:)==prn);									% 衛星PRN に対するエフェメリスの列インデックス
	if isempty(index), return;, end										% 最適なエフェメリスが無い場合は終了
	ttm=Eph_mat(23,index);												% エフェメリス送信時刻(tow)
	index=index(find(ttm<=tt.tow));										% エフェメリス送信時刻がエポック時刻より以前のインデックス
	if isempty(index), return;, end										% 最適なエフェメリスが無い場合は終了
% 	toe=Eph_mat(19,index);												% 軌道元期(tow) % 要検討
	ttm=Eph_mat(23,index);												% エフェメリス送信時刻(tow)
	week=Eph_mat(24,index);												% 週番号
% 	dt=abs((week-tt.week)*7*86400+(toe-tt.tow));						% 軌道元期との時間差の絶対値(towを利用, weekも考慮)
	dt=abs((week-tt.week)*7*86400+(ttm-tt.tow));						% エフェメリス送信時刻との時間差の絶対値(towを利用, weekも考慮)
	[mm,imin]=min(dt);													% 最近点のインデックス(indexの中で)
	
	fit=0.5;			% エフェメリス有効期間±0.5[h]要検討

	if mm<=fit*3600														% 有効期間以内にあるかどうかの判定
		column=index(imin);												% 最近点のインデックス(エフェメリスの中で)
	end
	if column~=0
		if Eph_mat(14,column)~=0										% 衛星のヘルスをチェック
			column=0;													% 最適なエフェメリスが無いとする
		end
	end
end
...
% data の並び --> L1, C1, P1, D1, L2, P2, D2, S1, S2 Androidように改変した
%--------------------------------------------
data = [];
for i = 1:9
	dat_ind = find(map==i);
