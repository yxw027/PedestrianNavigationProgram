...
% file I/O
% 
% SPP,PPP,VPPP(未),DGPPS,Float,Fix,All(Float+Fix)
% 
...
ax=newaxes(pos,xt,xl,xlm,yt,yl,ylm);									% New axes
% ここにプロットを挿入
plot(p.satprn{3}(:,1),p.satprn{3}(:,2),'color','b','linewidth',1);		% 可視衛星数のプロット
...
% 	end
	...
	p.dtr_vppp     =[result0.dtr,result1.dtr,result2.dtr];						% やなせ
	p.dtr      =result2.dtr;
	p.obs_rov.ele  =OBS.rov1.ele;
	p.obs_rov.azi  =OBS.rov1.azi;
end
...
set(p.handles.menu1(3),'separator','on');
...
p.opt.sat=1:32;
set(p.handles.menu4(1),'checked','on');
guidata(gcf,p);
