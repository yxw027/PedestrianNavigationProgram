function plot_trop(result,est_prm,dt)
%-------------------------------------------------------------------------------
% Function : 対流圏遅延用推定結果プロット
% 
% [argin]
% result  : 推定結果構造体(*.dtrop 1-2:ZTD or ZWD)
...
hold on
plot(result.time(:,4),result.dtrop,'.-');												% 対流圏遅延推定値のプロット
grid on
box on
...
ylabel('ZTD [m]');																		% Y軸のラベル
title(['Troposphere delay',' : ',TT]);													% タイトル
if size(result.dtrop,2)==2
	legend({'Rover Station','Reference Station'});										% 凡例
end
