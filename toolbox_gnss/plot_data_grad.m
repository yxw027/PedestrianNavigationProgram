...
if ~isempty(p.dion)
	if size(p.dion,2)==32
		...
		h=plot(p.esttime(:,4),p.dion,'.-');						% Fix解の点プロット
	end
end
ylabel(yls{nn});														% ylabel
xlabel('ToD [sec.]');													% xlabel
...
if fields(6)==1
	...
	if est_prm.statemodel.trop~=0
		p.dtrop    =Result.fix.dtrop_grad;
		p.dtrop(index_float,:)=Result.float.dtrop_grad(index_float,:);
	end
	if est_prm.statemodel.ion~=0
		p.dion     =Result.fix.dion_grad;
		p.dion(index_float,:)=Result.float.dion_grad(index_float,:);
	end
	p.satprn   =Result.fix.prn;
	p.obs_rov.ele  =OBS.rov.ele;
	p.obs_rov.azi  =OBS.rov.azi;
