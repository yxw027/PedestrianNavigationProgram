...
% 
%-------------------------------------------------------------------------------
% latest update : 2010/03/19 by Yanase, Ohashi, Tomita
%-------------------------------------------------------------------------------
% 
% ・異常値検定対応
% 
...
Result.float.dion(1:tt,1:32)=NaN;												% 電離層遅延(Zenith)
Result.float.dion_grad(1:tt,1:4)=NaN;											% 電離層遅延(Gradiant)
Result.float.dtrop(1:tt,1:2)=NaN;												% 対流圏遅延(Zenith)
Result.float.dtrop_grad(1:tt,1:4)=NaN;											% 対流圏遅延(Gradiant)
...
Result.fix.dion_grad(1:tt,1:4)=NaN;												% 電離層遅延(gradiant)
...
% 				CHI2.ref.ionl(timetag,:) = chi2_ref.ionl;					% 電離層(lglから算出,Nを含む)(ref)
		...
		if ~isnan(x1(1)) & length(prn.c)>3
			for nn=1:est_prm.iteration
				...
				switch est_prm.statemodel.trop		% yanase												% 対流圏遅延
				case 0, ix.T=[]; nx.x=nx.x+nx.T;													% 推定なし
				case 1, ix.T=nx.x+(1:nx.T); nx.x=nx.x+nx.T;											% ZWD推定
				case 2, ix.T=nx.x+(1:nx.T); nx.x=nx.x+nx.T;											% ZTD推定
				case 3, ix.T=nx.x+(1:nx.T); nx.x=nx.x+nx.T;											% ZWD+Grad推定
				case 4, ix.T=nx.x+(1:nx.T); nx.x=nx.x+nx.T;											% ZTD+Grad推定
				end
				...
				case 4, ix.i=nx.x+(1:ns+4);  nx.i=length(ix.i); nx.x=nx.x+nx.i;						% SDZID+Grad推定
				case 5, ix.i=nx.x+(1:2);  nx.i=length(ix.i); nx.x=nx.x+nx.i;						% ZID推定
				case 6, ix.i=nx.x+(1:4);  nx.i=length(ix.i); nx.x=nx.x+nx.i;						% ZID+dZID推定
				case 7, ix.i=nx.x+(1:6);  nx.i=length(ix.i); nx.x=nx.x+nx.i;						% ZID+Grad推定
				end
				...
				case 3, trop12p=[tzw1; 0; 0; tzw2; 0; 0];											% ZWD+Grad推定
				case 4, trop12p=[tzd1+tzw1; 0; 0; tzd2+tzw2; 0; 0];									% ZTD+Grad推定
				end
				...
				case 4, ion12p=[0; 0; 0; 0; (ion1(ii(b))./Mi1(ii(b))-ion2(ii(b))./Mi2(ii(b)))];		% SDZID+Grad推定
				case 5, ion12p=[1.0; 1.0];															% ZID推定
				case 6, ion12p=[1.0; 0; 1.0; 0];													% ZID+dZID推定
				case 7, ion12p=[1.0; 0; 0; 1.0; 0; 0];												% ZID+Grad推定
				end

				%--- カルマンフィルタの設定(2/2)と次元調節	% yanase
				%--------------------------------------------
				if timetag == 1 | isnan(Kalx_f(1)) % | timetag-timetag_o > 5						% 1エポック目(初期値設定)
					...
					switch est_prm.statemodel.trop
					case {1,2}
						Kalx_p=[Kalx_p; trop12p];													% 対流圏遅延(各局)
						KalP_p=blkdiag(diag(KalP_p(ix.u)),eye(nx.T)*est_prm.P0.std_dev_T(1));		% 初期共分散行列
					case {3,4}
						Kalx_p=[Kalx_p; trop12p];													% 対流圏遅延(各局)
						KalP_p=blkdiag(diag(KalP_p(ix.u)),diag(est_prm.P0.std_dev_T),diag(est_prm.P0.std_dev_T));			% 初期共分散行列
					end
					switch est_prm.statemodel.ion
					case {1,2,3,5,6}
						Kalx_p=[Kalx_p; ion12p];													% 電離層遅延
						KalP_p=blkdiag(KalP_p,eye(nx.i)*est_prm.P0.std_dev_i(1),...
								eye(nx.n)*est_prm.P0.std_dev_n).^2;									% 初期共分散行列
					case {4,7}
						Kalx_p=[Kalx_p; ion12p];													% 電離層遅延
						KalP_p=blkdiag(KalP_p,diag(est_prm.P0.std_dev_i(2:3)),diag(est_prm.P0.std_dev_i(2:3)),...
								eye(nx.i-4)*est_prm.P0.std_dev_i(1),eye(nx.n)*est_prm.P0.std_dev_n).^2;									% 初期共分散行列
					end
...
% 							eye(nx.i)*est_prm.P0.std_dev_i,eye(nx.n)*est_prm.P0.std_dev_n).^2;		% 初期共分散行列
				...
				case 2, 
					...
					trop12=TD*trop12;																% Troposphere(DD)
				case 3, 	% yanase
					Mgn1=Mw1.*cot(ele1).*cos(azi1); Mge1=Mw1.*cot(ele1).*sin(azi1);
					Mgn2=Mw2.*cot(ele2).*cos(azi2); Mge2=Mw2.*cot(ele2).*sin(azi2);
					trop1=Md1.*tzd1+Mw1.*Kalx_p(ix.T(1))...
							+Mgn1.*Kalx_p(ix.T(2))...
							+Mge1.*Kalx_p(ix.T(3));													% ZWD+Gradient推定用
					trop2=Md2.*tzd2+Mw2.*Kalx_p(ix.T(4))...
							+Mgn2.*Kalx_p(ix.T(5))...
							+Mge2.*Kalx_p(ix.T(6));													% ZWD+Gradient推定用
					trop12=trop1(ii(b))-trop2(ii(b));												% Troposphere(SD)
					Mwu=[TD*Mw1(ii(b)) TD*Mgn1(ii(b)) TD*Mge1(ii(b)) -TD*Mw2(ii(b)) -TD*Mgn2(ii(b)) -TD*Mge2(ii(b))];	% マッピング関数(行列)
					trop12=TD*trop12;																% Troposphere(DD)
				case 4, 	% yanase
					Mgn1=Mw1.*cot(ele1).*cos(azi1); Mge1=Mw1.*cot(ele1).*sin(azi1);
					Mgn2=Mw2.*cot(ele2).*cos(azi2); Mge2=Mw2.*cot(ele2).*sin(azi2);
					trop1=Md1.*tzd1+Mw1.*(Kalx_p(ix.T(1))-tzd1)...
							+Mgn1.*Kalx_p(ix.T(2))...
							+Mge1.*Kalx_p(ix.T(3));													% ZTD+Gradient推定用
					trop2=Md2.*tzd2+Mw2.*(Kalx_p(ix.T(4))-tzd2)...
							+Mgn2.*Kalx_p(ix.T(5))...
							+Mge2.*Kalx_p(ix.T(6));													% ZTD+Gradient推定用
					trop12=trop1(ii(b))-trop2(ii(b));												% Troposphere(SD)
					Mwu=[TD*Mw1(ii(b)) TD*Mgn1(ii(b)) TD*Mge1(ii(b)) -TD*Mw2(ii(b)) -TD*Mgn2(ii(b)) -TD*Mge2(ii(b))];	% マッピング関数(行列)
					trop12=TD*trop12;																% Troposphere(DD)
				end
...
% 					Miu=(Mi1(ii(b))+Mi2(ii(b)))/2;
					...
					ion12=MI*ion12;																	% Ionosphere(DD)
				case 4, 
					Fgn1=Mi1.*cot(ele1).*cos(azi1); Fge1=Mi1.*cot(ele1).*sin(azi1);
					Fgn2=Mi2.*cot(ele2).*cos(azi2); Fge2=Mi2.*cot(ele2).*sin(azi2);
					ion12=Kalx_p(ix.i);																% Ionosphere(SDZID+Gradient,estimate)
% 					Miu=(Mi1(ii(b))+Mi2(ii(b)))/2;
					Miu=Mi1(ii(b));
					MI=[-TD*Fgn1(ii(b)) -TD*Fge1(ii(b)) TD*Fgn2(ii(b)) TD*Fge2(ii(b)) -Miu(1)*ones(length(Miu)-1,1) diag(Miu(2:end))];		% 係数行列
					ion12=MI*ion12;																	% Ionosphere(DD)
				case 5, 
				...
				case 7, 
					Fgn1=Mi1.*cot(ele1).*cos(azi1); Fge1=Mi1.*cot(ele1).*sin(azi1);
					Fgn2=Mi2.*cot(ele2).*cos(azi2); Fge2=Mi2.*cot(ele2).*sin(azi2);
					ion1=Mi1.*Kalx_p(nx.u+nx.T+1)...
							+Fgn1.*Kalx_p(nx.u+nx.T+2)...
							+Fge1.*Kalx_p(nx.u+nx.T+3);												% ZID+Gradient推定用
					ion2=Mi2.*Kalx_p(nx.u+nx.T+4)...
							+Fgn2.*Kalx_p(nx.u+nx.T+5)...
							+Fge2.*Kalx_p(nx.u+nx.T+6);												% ZID+Gradient推定用
					ion12=ion1(ii(b))-ion2(ii(b));													% Ionosphere(SD)
					MI=[TD*Mi1(ii(b)) TD*Fgn1(ii(b)) TD*Fge1(ii(b)) -TD*Mi2(ii(b)) -TD*Fgn2(ii(b)) -TD*Fge2(ii(b))];	% マッピング関数(行列)
					ion12=TD*ion12;																	% Ionosphere(DD)
				end
...
% 				[Kalx_f, KalP_f] = filtsrcf_upd(zz, H, R, Kalx_p, KalP_p);
		...
		if ~isempty(zz)
			...
			case 4
				Result.float.dion(timetag,prn.u)=Kalx_f(ix.i(5:end));								% 電離層遅延(Zenith)
				Result.float.dion_grad(timetag,1:4)=Kalx_f(ix.i(1:4));								% 電離層遅延(Grad)
			case 5
				Result.float.dion(timetag,1:2)=Kalx_f(ix.i);										% 電離層遅延
			case 6
				Result.float.dion(timetag,1:4)=Kalx_f(ix.i);										% 電離層遅延
			case 7
				Result.float.dion(timetag,1:6)=Kalx_f(ix.i);										% 電離層遅延
			end
			if est_prm.statemodel.trop~=0
				switch est_prm.statemodel.trop
				case {1,2}
					Result.float.dtrop(timetag,1:2)=Kalx_f(ix.T);									% 対流圏遅延
				case {3,4}
					Result.float.dtrop(timetag,1:2)=Kalx_f(ix.T([1,4]));							% 対流圏遅延(Zenith)
					Result.float.dtrop_grad(timetag,1:4)=Kalx_f(ix.T([2,3,5,6]));					% 対流圏遅延(Grad)
				end
			end
		...
		if ~isempty(zz)
			if ~isnan(Fix_X(1))
				...
				case 4
					Result.fix.dion(timetag,prn.u)=Fix_X(ix.i(5:end));								% 電離層遅延(Zenith)
					Result.fix.dion_grad(timetag,1:4)=Fix_X(ix.i(1:4));								% 電離層遅延(Grad)
				case 5
					Result.fix.dion(timetag,1:2)=Fix_X(ix.i);										% 電離層遅延
				case 6
					Result.fix.dion(timetag,1:4)=Fix_X(ix.i);										% 電離層遅延
				case 7
					Result.fix.dion(timetag,1:6)=Fix_X(ix.i);										% 電離層遅延
				end
				if est_prm.statemodel.trop~=0
					switch est_prm.statemodel.trop	% yanase
					case {1,2}
						Result.fix.dtrop(timetag,1:2)=Fix_X(ix.T);									% 対流圏遅延
					case {3,4}
						Result.fix.dtrop(timetag,1:2)=Fix_X(ix.T([1,4]));							% 対流圏遅延
						Result.fix.dtrop_grad(timetag,1:4)=Fix_X(ix.T([2,3,5,6]));					% 対流圏遅延
					end
				end
...
% output_kml([est_prm.dirs.result,kmlname2],Result.float);
