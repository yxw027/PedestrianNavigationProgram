...
est_prm.stime  = '2008/12/13/03/00/00';					% Start time (例: '2007/01/01/00/00/00')
est_prm.etime  = '2008/12/13/03/30/00';					% End time   (例: '2007/01/01/24/00/00')

% データファイル設定
%--------------------------------------------
% ファイルは指定(上記の局番号は必ず設定すること, DLする場合は '' , [] にすること)
est_prm.file.rov_o = 'oem4_20081213.08O';				% Rov obs             (例: '****.07o')
est_prm.file.rov_n = 'oem4_20081213.08N';				% Rov nav(GPS)        (例: '****.07n')
est_prm.file.rov_g = '';				% Rov nav(GLONASS)    (例: '****.07g')
est_prm.file.ionex = '';								% IONEX      (例: 'igsg****.07i')
est_prm.file.sp3   = '';								% SP3        (例: 'igs*****.sp3')
est_prm.rovpos     = [-3755412.2819;  3634202.0881;  3644338.7927];	% Rover Pos  (例: [-3.7481114801E+06;  3.6358775482E+06;  3.6504372263E+06])
...
est_prm.obsmodel  = 6;									% 観測モデル [0:SPP(CA), 1:SPP(PY), 2:SPP(CA,PY), 3:GR(CA,L1), 4:UoC(CA,L1), 5:Trad(L1,L2), 6:GR(CA,PY,L1,L2), 7:UoC(CA,PY,L1,L2), 8:Trad(CA,PY,L1,L2), 9:GR(CA,L1,L2)]
est_prm.i_mode    = 2;									% 電離層モデル [1:Klobuchar, 2:GIM]
...
est_prm.mask2     = 70;                                  % 仰角マスク(ビルの谷間) [deg]
...
% 状態モデル
%--------------------------------------------
est_prm.statemodel.pos  = 0;							% 状態モデル(Position)    [0:static, 1:velocity, 2:accelelation, 3:jerk]
...
est_prm.statemodel.ion  = 3;							% 状態モデル(Ionosphere)  [0:OFF, 1:ZID, 2:ZID+dZID, 3:ZID+Grad]
est_prm.statemodel.trop = 4;							% 状態モデル(Troposphere) [0:OFF, 1:ZWD, 2:ZTD, 3:ZWD+Grad, 4:ZTD+Grad] ONの場合, 対流圏モデルは 4 に設定すること
