...
est_prm.stime  = '2017/12/18/07/06/11';					% Start time (例: '2007/01/01/00/00/00')
est_prm.etime  = '2017/12/18/07/07/39';					% End time   (例: '2007/01/01/24/00/00')

% データファイル設定
%--------------------------------------------
% ファイルは指定(上記の局番号は必ず設定すること, DLする場合は '' , [] にすること)
est_prm.file.rov_o = '2017_12_18_7_6.17o';			% Rov obs             (例: '****.07o')
est_prm.file.rov_n = '2017_12_18_7_6.17n';			% Rov nav(GPS)        (例: '****.07n')
est_prm.file.csv = '2017_12_18_7_6.csv';
...
est_prm.obsmodel  = 0;									% 観測モデル [0:SPP(CA), 1:SPP(PY), 2:SPP(CA,PY), 3:GR(CA,L1), 4:UoC(CA,L1), 5:Trad(L1,L2), 6:GR(CA,PY,L1,L2), 7:UoC(CA,PY,L1,L2), 8:Trad(CA,PY,L1,L2), 9:GR(CA,L1,L2)]
...
est_prm.obsnoise.SENSOR_E = 0.05^2;
est_prm.obsnoise.SENSOR_N = 0.05^2;
est_prm.obsnoise.SENSOR_U = 0.05^2;

est_prm.obsnoise.WARKING = 0.5^2;                       %歩幅の分散

% 状態モデル
%--------------------------------------------
est_prm.statemodel.pos  = 1;							% 状態モデル(Position)    [0:static, 1:velocity, 2:accelelation, 3:jerk]
...
est_prm.statemodel.std_dev_u = 0.2;						% 位置
est_prm.statemodel.std_dev_vx = 0.1;					% 速度 %STATE用
est_prm.statemodel.std_dev_vy = 0.1;					% 速度 %STATE用
...
est_prm.statemodel.std_walking = 0.1;               %%歩幅
...
est_prm.P0.std_dev_t = [20,20];							% 受信機時計誤差(dtr,dtr_dot)
...
est_prm.P0.std_walking = 1.0;                     % 歩幅[E方向, N方向]
