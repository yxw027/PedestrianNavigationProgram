...
%         end
		...
		if find([0,1,2,10]==est_prm.obsmodel)
			...
			if timetag==1 || isnan(Kalx_f(1)) || timetag-timetag_o > 5 ||timetag==78&timetag<88 %ここ
                ...
                %INSsensor(1:3)=getSensortameshi(fpcsv,time,timetag);
		...
		if find([3,4,5,6,7,8,9]==est_prm.obsmodel)
			...
			%--- 衛星が変化した場合に次元を調節する
			%--------------------------------------------
			if timetag == 1 | isnan(Kalx_f(1)) | timetag-timetag_o > 5	||timetag>=78&timetag<88							% 1エポック目
...
% 				[Kalx_f, KalP_f, gam] = filthif_upd(zz, H, R, Kalx_p, KalP_p, 5);
        ...
        est_poscheck(timetag,:)=xyz2enu(Kalx_f(1:3),refl)';
        %ここからためし    
%          if timetag==50
%         est_pos1(1,:)=xyz2enu(Kalx_f(1:3),refl)';% ENUに変換
%         AAAA=est_pos1(1,:);
%         est_counter=1;
%          end
%         INS1(50:71,1:3)=[-1.980674 0.277299 0
%             -1.982535 0.263391 0
%             -1.984871	0.245007	0
% -1.982168	0.266465	0
% -0.989414	0.145121	0
...
% -1.981007	0.273062	0
% ];
%INS1(72:79,1:3)=[-1.979499	0.285625	0
%-1.988806	0.209797	0
%-1.994187	0.149934	0
% -0.996992	0.077504	0
% -1.993487	0.161044	0
...
%        if timetag>=51
%            if est_counter==1
%                 est_pos2(est_counter,:)=est_pos1(1,:);
%            end
%        est_pos2(est_counter+1,:)=est_pos2(est_counter,:)+INS1(35+est_counter,:); 
%      est_pos3(est_counter,:)=enu2xyz(est_pos2(est_counter+1,1:3)',refl);
%       est_posllh(est_counter,:)=xyz2llh(est_pos3(est_counter,1:3))*180/pi;
...
% 		if change_flag==1, fprintf(' , Change');, end
		...
		time_o=time;
		timetag_o=timetag;
 ABCD(timetag,1:3)=[Result.ppp.pos(5),Result.ppp.pos(4),Result.ppp.pos(6)];
	end
	% end 判定
	%--------------------------------------------
